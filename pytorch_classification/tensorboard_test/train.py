...
def main(args):
    ...
    # 实例化SummaryWriter对象 参数：tensorboard文件的保存位置
    ...
    # 将模型写入tensorboard [创建一个0矩阵：init_img]
    init_img = torch.zeros((1, 3, 224, 224), device=device)
    # add_graph函数：将模型 + init_img传入
    ...
    lf = lambda x: ((1 + math.cos(x * math.pi / args.epochs)) / 2) * (1 - args.lrf) + args.lrf  # cosine 余弦
    ...
    for epoch in range(args.epochs):
        ...
        # add figure into tensorboard (图片添加)
        # data_utils脚本
        ...
        tb_writer.add_histogram(tag="conv1",
                                values=model.conv1.weight,  # 模型权重
                                global_step=epoch)
        # layer1/block0/conv1
