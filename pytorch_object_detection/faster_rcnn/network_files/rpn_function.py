...
class AnchorsGenerator(nn.Module):
    ...
    # 初始化函数
    # sizes：anchor的scale
    # aspect_ratios：anchor的不同比例
    # 这里的默认值暂时没有用到
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0)):
        super(AnchorsGenerator, self).__init__()

        # 判断size的每个元素是不是list或tuple类型
        if not isinstance(sizes[0], (list, tuple)):
            # TODO change this
            sizes = tuple((s,) for s in sizes)
        # 判断aspect_ratios的每个元素是不是list或tuple类型
        if not isinstance(aspect_ratios[0], (list, tuple)):
            aspect_ratios = (aspect_ratios,) * len(sizes)

        # 判断size与aspect_ratios的元素个数是否一致
        ...
        # 空字典，会把原图上所有anchor的坐标信息存入
        self._cache = {}

    # 生成anchor模板
    # 传入 scales, aspect_ratios
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        Arguments:
            ...
            device: cpu/gpu
        """
        # 将scales由list转换为tensor
        scales = torch.as_tensor(scales, dtype=dtype, device=device)
        # 将 aspect_ratios转换为tensor
        aspect_ratios = torch.as_tensor(aspect_ratios, dtype=dtype, device=device)
        # 将 aspect_ratios开根号 = 高度的乘法因子
        h_ratios = torch.sqrt(aspect_ratios)
        # 宽度的乘法因子 = 1/高度乘法因子
        w_ratios = 1.0 / h_ratios

        # [r1, r2, r3]' * [s1, s2, s3]
        # number of elements is len(ratios)*len(scales) - anchor的个数
        # 计算每个anchor的高度和宽度
        # 乘法因子+新维度 * scales+新维度，通过view展平
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            # 生成一个序列：从0开始，元素个数 = grid_width，将序列 * stride_width
    ...
    def cached_grid_anchors(self, grid_sizes, strides):
        ...
        # 将grid_sizes，strides转换为字符形式，相加得到Key值
        key = str(grid_sizes) + str(strides)
        # self._cache是字典类型
        # 查询key值是否在字典（空）中
        if key in self._cache:
            return self._cache[key]
        # 将grid_sizes, strides传递给grid_anchors
        anchors = self.grid_anchors(grid_sizes, strides)
        self._cache[key] = anchors
        return anchors

    # 正向传播过程
    # image_list：ImageList类，存储预处理过程中的图片batch及图像的尺寸信息
    # feature_maps：预测特征层的信息，List[Tensor]，list的个数：预测特征层的个数（比如：1或者5）
    def forward(self, image_list, feature_maps):
        ...
        # 计算特征层上的一步等于原始图像上的步长
        # 遍历grid_sizes（每个预测特征层的大小），将图像大小//特征矩阵大小
        # 变量类型：torch.int64
        # 组合成一个list
        ...
        # 根据提供的sizes和aspect_ratios生成anchors模板
        # anchor的高度和宽度对应的都是原图的尺度
        self.set_cell_anchors(dtype, device)

        # 计算/读取所有anchors的坐标信息（这里的anchors信息是映射到原图上的所有anchors信息，不是anchors模板）
        # 得到的是一个list列表，对应每张预测特征图映射回原图的anchors坐标信息
        # grid_sizes：预测特征层 高度宽度信息
        # strides：尺度信息
...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        # 3x3 滑动窗口
        # in_channels：输入特征矩阵的channel（卷积核的个数）
        # in_channels： 输出特征矩阵的channel = 输入特征矩阵的channel
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        # 预测器：计算预测的目标分数（这里的目标只是指前景或者背景）
        # num_anchors：卷积核个数 = 预测特征层中每一个cell中anchor的个数（原论文是2k）
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 预测器：计算预测的目标bbox regression参数
        # num_anchors * 4：anchor的个数*4（中心坐标的xy偏移量，高度宽度偏移量）
    ...
    def forward(self, x):
        ...
        logits = []
        bbox_reg = []
        # 遍历每一个预测特征层（有些backbone生成多个预测特征层）
        for i, feature in enumerate(x):
            # 通过滑动窗口和relu激活函数得到输出特征矩阵
            t = F.relu(self.conv(feature))
            # 分别通过两个预测器，将结果添加入两个列表
...
def permute_and_flatten(layer, N, A, C, H, W):
    ...
    # N：batch，-1：这个维度数字会自动推理，C：classes，H,W）
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    """
    # 两个空列表，存储预测目标分数、预测bbox回归参数
    box_cls_flattened = []
    box_regression_flattened = []

    # 遍历每个预测特征层
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        #[batch，A*C：anchor个数*classes个数，height, width]
        ...
        # [N, -1, C]
        # permute_and_flatten：对参数进行顺序调整及展平处理
        box_cls_per_level = permute_and_flatten(box_cls_per_level, N, A, C, H, W)
        box_cls_flattened.append(box_cls_per_level)

        # [N, -1, C]
        # permute_and_flatten：对参数进行顺序调整及展平处理
        # 调整原因：方便后面将预测值与anchor进行结合
        box_regression_per_level = permute_and_flatten(box_regression_per_level, N, A, 4, H, W)
        box_regression_flattened.append(box_regression_per_level)

    # 对列表box_cls进行拼接（将多个预测特征层上的信息进行拼接），并进行展平
    box_cls = torch.cat(box_cls_flattened, dim=1).flatten(0, -2)  # start_dim, end_dim
    # 对列表box_regression进行拼接，并进行展平
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head,
                 # rpn计算损失时，采集增幅样本设置的阈值，fg:前景，bg:背景
                 fg_iou_thresh, bg_iou_thresh,
                 batch_size_per_image, positive_fraction,
                 pre_nms_top_n, post_nms_top_n, nms_thresh, score_thresh=0.0):
        super(RegionProposalNetwork, self).__init__()
        # TODO self赋值的相关问题
        # 将传入的anchor_generator赋值给self……
        self.anchor_generator = anchor_generator
        self.head = head
        # box_coder类的实例化，赋值给self……
        self.box_coder = det_utils.BoxCoder(weights=(1.0, 1.0, 1.0, 1.0))

        # use during training
        # box_iou：计算anchors与真实bbox的iou
        ...
        # 类BalancedPositiveNegativeSampler
    ...
    def forward(self,
        ...
        # features是所有预测特征层组成的OrderedDict（将预测特征层的特征矩阵都提取出来（提取values，不要key））
        features = list(features.values())

        # 计算每个预测特征层上的预测目标概率和bboxes regression参数
        # objectness和pred_bbox_deltas都是list
        # 将预测特征层输入head类（3*3+并行2 1*1，对特征矩阵进行预测）
        ...
        # 计算每个预测特征层上的对应的anchors数量（每个cell：15个anchor，特征矩阵的高度：34，宽度：42）
        ...
        # the proposals
        # decode：将预测的bbox regression参数应用到anchors上得到最终预测bbox坐标（proposal坐标）
