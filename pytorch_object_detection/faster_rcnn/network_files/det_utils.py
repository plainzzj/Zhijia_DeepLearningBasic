...
class BoxCoder(object):
    ...
    *** 该类将一组边界框编码并解码为用于训练回归器的表示。
    ...
    def decode(self, rel_codes, boxes):
        ...
        Args:
            rel_codes: bbox regression parameters（预测边界框的回归参数）
            boxes: anchors/proposals（anchor坐标）

        Returns:

        """
        # 判断boxes，rel_codes是否为所指定的类型
        assert isinstance(boxes, (list, tuple))
        assert isinstance(rel_codes, torch.Tensor)
        # 遍历boxes（八张图片）的维度0:21420个anchor个数
        # 每个batch生成的anchor个数是不一定的……
        boxes_per_image = [b.size(0) for b in boxes]
        # 对boxes进行拼接（将一个batch中所有anchor信息拼接在一起）
        concat_boxes = torch.cat(boxes, dim=0)

        # 求batch中所有anchor信息的综述
        box_sum = 0
        for val in boxes_per_image:
            box_sum += val

        # decode_single：将预测的bbox回归参数应用到对应anchors上得到预测bbox的坐标
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        """
        # 将boxes变量设置成为与rel_codes一样的设备及数据类型
